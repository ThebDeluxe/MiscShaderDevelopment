...
public class DentContactSource : MonoBehaviour
{
    ...
    [Tooltip("Only accept contacts generated by this collider. Leave empty to accept any " +
             "collider on the body.\n\n" +
             "Worth setting: a Rigidbody owns every collider beneath it that has no body of " +
             "its own, so stray colliders elsewhere in the hierarchy will otherwise feed " +
             "contacts in here and make the dent sources jump around.")]
    ...
    void Harvest(Collision collision)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Ignore contacts from colliders we were not told to listen to.
            if (contactCollider != null && cp.thisCollider != contactCollider) continue;
